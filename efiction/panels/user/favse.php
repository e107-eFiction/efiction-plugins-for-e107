...
if (($add || $edit || $delete) && !isMEMBER) {
    ...
    if ($delete) {
    ...
    } elseif ($edit && isset($_POST['submit'])) {
    ...
    } elseif (($add || $edit) && !isset($_POST['submit'])) {
        ...
        $tpl->assign('pagelinks', '<form method="POST" enctype="multipart/form-data" action="member.php?action=favse&amp;add=1&amp;seriesid='.($add ? $add : $edit)."\">\n
		...
		$seriescount = dbquery('SELECT COUNT(item) FROM '.TABLEPREFIX."fanfiction_favorites WHERE uid = '$uid' AND type = 'SE' GROUP BY uid");
