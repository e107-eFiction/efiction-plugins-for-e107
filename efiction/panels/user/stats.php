...
if(empty($stat)) $output .= "<div class='authorstats'><span class='ff-label'>"._AUTHOROF."</span> ". implode(", ", $authorof)."</div>";
if($favorites) {
	...
	if(empty($stat)) $output .= "<div class='authorstats'><span class='ff-label'>"._FAVOF.": </span> <a href='".$thislink."action=stats&amp;stat=favauthor'>$favcount "._MEMBERS."</a><br /><span class='ff-label'>".(USERUID == $uid ? _YOURFAVORITES : _FAVORITESOF." $penname").": </span>".implode(", ", $favlist)."</div>";
}
?>