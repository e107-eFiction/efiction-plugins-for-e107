...
if ($_GET['action'] == "newstory" || $_GET['action'] == "editstory") {
    $current = "addstory";
}
...
if (!isMEMBER || ($submissionsoff && !isADMIN) || (!isADMIN && isset($uid))) {
    accessDenied();
}
if (!isADMIN || uLEVEL > 3) {
    if (isset($chapid)) {
    ...
    if (isset($sid)) {
        ...
        if ($story['uid'] != USERUID && (is_array($coauthors) && !in_array(USERUID, $coauthors)) && !$story['rr']) {
            accessDenied();
        }
    }
} elseif (isADMIN && uLEVEL < 4 && isset($_GET['admin'])) {
    $admin = 1;
...
function preview_story($stories)
{
    ...
    if (file_exists("$skindir/listings.tpl")) {
        $tpl = new TemplatePower("$skindir/listings.tpl");
    } else {
        $tpl = new TemplatePower(_BASEDIR."default_tpls/listings.tpl");
    }
    if (count($stories['coauthors']) > 0) {
        $stories['coauthors'] = 1;
    }
    $tpl->prepare();
    ...
    if (!empty($stories['storytext'])) {
        ...
        if (file_exists("$skindir/viewstory.tpl")) {
            $tpl = new TemplatePower("$skindir/viewstory.tpl");
        } else {
...
function newstory($current = '')
{
    ...
    if (isset($_FILES['storyfile']['name']) && $_FILES['storyfile']['name']) {
        if ($_FILES['storyfile']['type'] != 'text/html' && $_FILES['storyfile']['type'] != 'text/plain') {
            ...
            foreach ($texts as $text) {
                if ($_FILES['storyfile']['type'] == 'text/html') {
                    $story .= rtrim($text, "\n\r\t")." ";
                } else {
                    $story .= $text;
                }
    ...
    if (isset($_POST['submit'])) {
        ...
        if (find_naughty($title)) {
            $failed .= "<br />"._NAUGHTYWORDS;
        }
        if (($minwords && $wordcount < $minwords) || ($maxwords && $wordcount > $maxwords)) {
            $failed .= "<br />"._WORDCOUNTFAILED;
        }
        $storyvalid = 0;
        if ($newchapter) {
    ...
    if (isset($_POST['submit']) && $_POST['submit'] == _ADDSTORY && !isset($submit)) {
        ...
        if (!$validated && (($autovalidate && !isADMIN) || $user['validated'] || $storyvalid == 2)) {
            $validated = 1;
        } elseif (!$validated) {
            $validated = 0;
        }
        ...
        if ($store == "db") {
            ...
            if (($newchapter && $insertchapter != false) || $insertstory != false) {
                ...
                $output .= write_error(_FATALERROR." "._TRYAGAIN);
            }
        }
        if (!$newchapter) {
            if (count($coauthors)) {
    ...
    if (isset($_POST['submit']) || $newchapter) {
    ...
    if (!$newchapter) {
        $output .= storyform($stories, $submit);
    }
...
function viewstories()
{
    ...
    if (($go || $com) && $sid) {
        ...
        if ($com) {
            e107::getDb()->gen("UPDATE ".TABLEPREFIX."fanfiction_stories SET completed = ".($com == "yes" ? "1" : "0")." WHERE sid = '$sid'");
        }
    }
    ...
    foreach ($stories as $story) {
        ...
        $output .= "<tr><td class=\"tblborder\"><a href=\"viewstory.php?sid=".$story['sid']."\">".stripslashes($story['title'])."</a> ".ratingpics($story['rating'])." <strong>"._COMPLETE.":</strong> <a href=\"stories.php?action=viewstories&amp;sid=".$story['sid']."&amp;com=".$story['completed']."\"><a href=\"stories.php?action=viewstories&amp;sid=".$story['sid']."&amp;com=".($story['completed'] == 1 ? "no\">"._YES : "yes\">"._NO)."</a></td>
			...
			";
        if ($reviewsallowed) {
            $output .= "<td class=\"tblborder\" align=\"center\">".($story['reviews'] ? "<a href=\"reviews.php?type=ST&amp;item=".$story['sid']."\">".$story['reviews']."</a>" : "0")."</td>";
        }
        if (!$autovalidate) {
            $output .= "<td class=\"tblborder\" align=\"center\">".($story['validated'] > 0 ? _YES : _NO)."</td>";
        }
        ...
        if ($hidechapters && $hidechapters != "hide") {
            foreach ($chapters as $chapter) {
                ...
                if ($num_chapters > 1) {
                    $output .= "<td  class=\"tblborder\" align=\"center\"><a href=\"stories.php?action=viewstories&amp;go=up&amp;sid=".$story['sid']."&amp;chapid=".$chapter['chapid']."&amp;inorder=".$chapter['inorder']."&amp;chapters=view\">$up</a></td>
                ...
                if ($num_chapters > 1) {
                    $output .= " - <a href=\"stories.php?action=delete&amp;chapid=".$chapter['chapid']."&amp;sid=".$story['sid']."\">"._DELETE."</a>";
                }
                $output .= "</td><td class=\"tblborder\" align=\"center\">";
                if ($reviewsallowed) {
                    $output .= ($chapter['reviews'] ? "<a href=\"reviews.php?type=ST&amp;item=".$story['sid']."&amp;chapid=".$chapter['chapid']."\">".$chapter['reviews']."</a>" : "0");
                }
                if (!$autovalidate) {
                    $output .= "</td><td class=\"tblborder\" align=\"center\">".($chapter['validated'] > 0 ? _YES : _NO);
                }
...
function editchapter($chapid)
{
    ...
    if (isset($_POST['submit'])) {
        ...
        $storytext = e107::getParser()->toDb($storytext, "DESCRIPTION");
        
        $inorder = $_POST["inorder"];  

		$words = e107_wordscount($storytext);   
        // 2 down because html + /html, toText() wasn't used
        $wordcount = count($words) - 2 ;
    } 
    ...
    if (isset($_POST['submit']) && $_POST['submit'] == _ADDSTORY) {
        if (!$chaptertitle || !$storytext) {
            ...
            if ($admin && empty($_POST["uid"])) {
                ...
                $uid = $user['uid'];
            } elseif (isset($_POST['uid']) && isNumber($_POST['uid'])) {
                $uid = $_POST['uid'];
            } else {
                $uid= USERUID;
            }
...
function editstory($sid)
{
    ...
    if (isset($_POST['submit'])) {
        ...
        if (!$admin && $authorvalid && !$validated) {
            $validated = 2;
        }
        $au[] = $uid;
        if (count($coauthors)) {
    ...
    if (isset($_POST['submit']) && $_POST['submit'] == _ADDSTORY) {
        ...
        if (!$rid) {
            ...
            if ($admin) {
                ...
                if ($olduid != $uid) {
                    if ($store == "files") {
                        ...
                        $chapupdate = e107::getDb()->gen("UPDATE ".TABLEPREFIX."fanfiction_chapters SET uid = '$uid' WHERE sid = '$sid' AND uid = '$olduid'");
                    }
            ...
            } elseif ($admin && !$validated && $oldvalid > 0) {
    ...
    while ($chapter = dbassoc($chapquery)) {
        ...
        if ($num_chapters > 1) {
            $output .= "<td align=\"center\" class=\"tblborder\">".($chapter['inorder'] == 1 ? "" : "<a href=\"stories.php?action=viewstories&amp;go=up&amp;sid=$sid&amp;chapid=".$chapter['chapid']."&amp;inorder=".$chapter['inorder']."\">$up</a>").
        ...
        if ($num_chapters > 1) {
            $output .= " - <a href=\"stories.php?action=delete&amp;chapid=".$chapter['chapid']."&amp;sid=$sid".($admin ? "&amp;admin=1&amp;uid=".$chapter['uid'] : "")."\">"._DELETE."</a>";
        }
        $output .= "</td></tr>";
    }
...
function delete()
{
    ...
    if (!$sid && !$chapid) {
        return write_error(_ERROR);
    }
    $caption = "<div id=\"pagetitle\">".($chapid ? _DELETECHAPTERTITLE : _DELETESTORYTITLE)."</div>";
    if ($admin) {
        if ($chapid) {
            $authorquery =  "SELECT uid FROM ".TABLEPREFIX."fanfiction_chapters WHERE chapid = '$chapid' LIMIT 1" ;
        } else {
            $authorquery =  "SELECT uid FROM ".TABLEPREFIX."fanfiction_stories WHERE sid = '$sid' LIMIT 1" ;
        }
