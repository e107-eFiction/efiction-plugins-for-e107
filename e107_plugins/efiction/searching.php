...
if (isset($_GET['action']) && $_GET['action'] == 'advanced') {
    $displayform = 1;
}
// Now set up the page
...
e107::getRender()->tablerender($caption, $output, $current);;
    ...
    if (isset($_REQUEST['charlist1'])) {
        $charid[] = $_REQUEST['charlist1'];
    }
    if (isset($_REQUEST['charlist2'])) {
        $charid[] = $_REQUEST['charlist2'];
    }
    ...
    if (count($charid) > 0) {
        ...
        if (count($charseries) > 0) {
            $scountquery[] = '('.implode(' AND ', $charseries).')';
        }
    }

    if (isset($_REQUEST['rid'])) {
        $rid = is_array($_REQUEST['rid']) ? $_REQUEST['rid'] : explode(',', $_REQUEST['rid']);
    } else {
        $rid = array();
    }
    ...
    foreach ($classtypelist as $id => $vars) {
        $opts = array();
        $exopts = array();
        if (isset($_POST['class_'.$id])) {
            $opts = array_filter($_POST['class_'.$id], 'isNumber');
        } elseif (isset($_GET['class_'.$id])) {
            $opts = array_merge($opts, explode(',', $_GET['class_'.$id]));
            $opts = array_filter($opts, 'isNumber');
        } elseif (isset($_REQUEST[$vars['name']])) {
            $opts[] = array($_REQUEST[$vars['name']]);
        }
        $classin = array_merge($classin, $opts);
        if (isset($_POST['exclass_'.$id])) {
            $exopts = array_filter($_POST['exclass_'.$id], 'isNumber');
        } elseif (isset($_GET['exclass_'.$id])) {
    ...
    if ($wordlow != '-500') {
        $wordcount = 'stories.wordcount > '.$wordlow;
    }
    if ($wordhigh != '1000000') {
        if ($wordcount) {
            $wordcount .= ' AND stories.wordcount < '.$wordhigh;
        } else {
            $wordcount = 'stories.wordcount < '.$wordhigh;
        }
    }
    if ($wordhigh < $wordlow) {
        $wordcount = 'stories.wordcount < '.$wordlow;
    ...
    if (count($scountquery) > 0) {
        $scountquery = ' WHERE '.implode(' AND ', $scountquery);
    }
    ...
    foreach ($termArray as $term => $value) {
        if ($term == 'submit' || $term == 'wordhigh' || $term == 'wordlow' || empty($value) || $term == 'formname' || $term == 'searchtype') {
            continue;
        }
    ...
    if (!empty($scountquery)) {
        ...
        if ($scount > 0) {
            $otherresults[] = "<a href='browse.php?type=series$terms'>$scount "._SERIES.'</a>';
        }
    }
    ...
    if (count($otherresults) > 0) {
        $tpl->assign('otherresults', "<div id='otherresults'><span class='label'>"._OTHERRESULTS.':</span> '.implode(', ', $otherresults).'</div>');
    }
} else {
