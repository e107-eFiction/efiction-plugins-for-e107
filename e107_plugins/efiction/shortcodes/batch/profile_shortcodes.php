...
class plugin_efiction_profile_shortcodes extends e_shortcode
{
	...
	{
        if(!empty($this->var['user_signature'])) return e107::getParser()->toHTML($this->var['user_signature'], TRUE);
		else {
			return !empty($this->var['bio']) ? e107::getParser()->toHTML($this->var['bio'], TRUE) : null;
		}
	...
	public function sc_profile_realname($parm = null)
	{
        ...
        if($this->var['realname']) 	return $this->var['realname'];
        return '';
    }
    /* {PROFILE_REPORTTHIS} */
	public function sc_profile_reportthis($parm = null)
	{
      ...
      $text=
      $start."<a ".$class." href=\"".e_PLUGIN_ABS."efiction/report.php?action=report&amp;url=viewuser.php?uid=".$this->var['uid']."\">"._REPORTTHIS."</a>".$end;
      return $text;
	}
    /* {PROFILE_USERLEVEL} */
