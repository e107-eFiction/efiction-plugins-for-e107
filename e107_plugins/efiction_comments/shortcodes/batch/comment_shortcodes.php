...
class plugin_efiction_comments_comment_shortcodes extends e_shortcode
{
	var $var;
	...
	//	print_a($this->var);
		...
		if(USERID || $pref['anon_post'] == 1)
		{
			if(isset($this->var['comment_lock']) && $this->var['comment_lock'] != "1" && $this->var['comment_blocked'] < 1)
			{
				...
				{
					$REPLY = "<a id='efiction-comment-reply-".$this->var['comment_id']."' class='efiction-comment-reply btn btn-default btn-secondary btn-sm btn-mini btn-xs' data-type='".$this->var['comment_type']."' data-target='".EFICTION_COMMENTS_DIR."comments.php' href='".EFICTION_COMMENTS_DIR."comments.php?reply.".$thistable.".".$this->var['comment_id'].".".$thisid."'>".COMLAN_326."</a>";
				}
			}
		}
		return $REPLY;
	...
	//	e107::getDebug()->log($this->var);
		...
		{
			$text .= "<a href='#' data-target='" . EFICTION_COMMENTS_DIR."comments.php' id='efiction-comment-approve-" . $this->var['comment_id'] . "' class='efiction-comment-approve btn btn-default btn-secondary btn-sm btn-mini btn-xs'>" . COMLAN_404 . "</a> ";
		}
		return $text;
		/*
		$url 		= e_PAGE."?".e_QUERY;
	...
	function sc_comment_button($parm = null)
	{
		...
		if($this->mode == 'edit')
		{
			...
			$class = "efiction-comment-submit ";
			$class .= (!empty($parm['class'])) ? $parm['class'] : 'button btn btn-primary efiction-comment-submit pull-right float-end float-right';
			$options = array(
				'class'         => $class,
				'data-pid'      => $pid,
				'data-sort'     => $pref,
				'data-target'   => EFICTION_COMMENTS_DIR . 'comments.php',
	...
	*/
	...
	function sc_comment_edit($parm = null)
	{
		...
		if ($pref['allowCommentEdit'] && USER && $this->var['user_id'] == USERID && ($this->var['comment_lock'] < 1))
		{
			...
			{
				return "<a data-target='".EFICTION_COMMENTS_DIR."comments.php' id='efiction-comment-edit-".$this->var['comment_id']."' class='btn btn-default btn-secondary btn-sm btn-mini btn-xs efiction-comment-edit' href='".e_SELF."?".e_QUERY."&amp;comment=edit&amp;comment_id=".$this->var['comment_id']."'>{$adop_icon}</a>";
			}
			else
			{
				...
				return "<a data-target='".SEFICTION_COMMENTS_DIR."comments.php' id='efiction-comment-edit-".$this->var['comment_id']."' class='btn btn-default btn-secondary btn-sm btn-mini btn-xs efiction-comment-edit' href='".EFICTION_COMMENTS_DIR."comments.php?".$comment_edit_query.".edit.".$this->var['comment_id']."#efiction-comment-form'>".$adop_icon."</a>";
			}
		}
		else
		{
