...
/*  NOTE:  this is just copy of e107 core comment handler */
/*  user_id  = author ID */
    ...
    class efiction_comments
{
	...
	function __construct()
	{
		...
		{		
			...
			$COMMENT_TEMPLATE =  e107::getTemplate('efiction_comments', 'comment');
		}
	...
	function replyComment($id) // Ajax Reply. 
	{
		...
		$sql = e107::getDb();
		if($sql->select("fanfiction_comments","*","comment_id= ".intval($id)." LIMIT 1"))
		{
			$row = $sql->fetch();
			// [comment_id] =&gt; 65
	...
	// 	require_once(e_HANDLER."ren_help.php");
		...
		if ($this->getCommentPermissions() == 'rw')
		{
			...
			{ // Get existing comment
				$id = intval($id);
				$sql->select("fanfiction_comments", "*", "comment_id='{$id}' ");
				$ecom = $sql->fetch();
			...
			*/
			...
			$formid = ($action == 'reply') ? "efiction-comment-form-reply" : "efiction-comment-form";
		...
		//	$text .= "Indent = ".$indent;
            ...
            $sc = e107::getScParser()->getScObject('comment_shortcodes', 'efiction_comments', false);
...
//		$ns = new e107table;
		...
		$comment_shortcodes = $sc = e107::getScParser()->getScObject('comment_shortcodes', 'efiction_comments', false);
	...
	function approveComment($id) // appropve a single comment by comment id.  
	{
		...
		return e107::getDb()->update("fanfiction_comments","comment_blocked=0 WHERE comment_id = ".intval($id)."");
	}
	...
	//	if(THEME_LEGACY !== true) // old themes might still use bbcodes.
		...
		if(!e107::getDb()->update("fanfiction_comments","comment_comment=\"".$tp->toDB($comment)."\" WHERE comment_id = ".intval($id).""))
		{
	...
	//	if(THEME_LEGACY !== true) // old themes might still use bbcodes.
		...
		if (!$sql->select("fanfiction_comments", "*", "comment_comment='".$comment."' AND comment_item_id='".intval($id)."' AND comment_type='".$tp->toDB($type, true)."' "))
		{
			if ($_POST['comment'])
			{
				if (USER == TRUE)
				...
				if (!defined("emessage"))
				{
					...
					if ($editpid)
					{
						...
						$sql->update("fanfiction_comments", "comment_comment='{$comment}' WHERE comment_id='".intval($editpid)."' ");
					...
					if (!($inserted_id = $sql->insert("fanfiction_comments", $edata_li, true)))
					{
...
/*
	...
	function count_comments($table, $id)
	{
		...
		$type = $this->getCommentType($table);
		$count_comments = $sql->count("fanfiction_comments", "(*)", "WHERE comment_item_id='".intval($id)."' AND comment_type='".$tp->toDB($type, true)."' ");
	 ...
	 * @return null|string
	 */
	public function render($plugin, $id, $subject, $rate=false, $return = 'html', $tablerender = TRUE)
	{
		return $this->compose_comment($plugin, 'comment', $id, 0, $subject, $rate, $return, $tablerender );
...
// ------------- TODO move the 'listing' into separate function so that ajax can access it easily. 
		...
		if($text)
		{
			...
			if($this->totalComments && getperms("B"))
			{
				...
				//	$modcomment .= "<a href='".e_ADMIN_ABS."modcomment.php?$table.$id'>".COMLAN_314."</a>";
                    $url = e_PLUGIN_ABS."efiction_comments/admin_comments.php";
					$modcomment .= "<a class='btn btn-default btn-secondary btn-mini btn-sm' href='".$url."?searchquery={$id}&filter_options=comment_type__".$this->getCommentType($table)."'>".COMLAN_314."</a>";
	...
	//	$tablerender = true;
		...
		{		
			if ($tablerender)
			{
					...
					echo $ns->tablerender("<span id='efiction-comment-total'>".$this->totalComments."</span> ".LAN_COMMENTS, $TEMPL, 'comment', TRUE);
			}
			else
			{
				echo $TEMPL;	
		...
		elseif($return === 'html' OR $return )
		{
			if ($tablerender)
			{
				return $ns->tablerender("<span id='efiction-comment-total'>".$this->totalComments."</span> ".LAN_COMMENTS, $TEMPL, 'comment', true);
			}
			else {
		...
		$ret['caption'] = "<span id='efiction-comment-total'>".$this->totalComments."</span> ".LAN_COMMENTS;
	...
	public function getComments($table,$id,$from=0,$att=null)
	{
		...
		if(!empty($pref['nested_comments']))
		{
			$query = "SELECT c.*, u.*, ue.*, r.* FROM #fanfiction_comments AS c
	...
	function nextprev($table,$id,$from=0)
	{
		...
		$navid = 'efiction-comments-nav-'.e107::getForm()->name2id($table);
	...
	function recalc_user_comments($id)
	{
		...
		function delete_comments($table, $id)
		{
			...
			$num_deleted = $sql->delete("fanfiction_comments", "comment_item_id='{$id}' AND comment_type='{$type}'");
		...
		function getCommentData($amount = '', $from = 0, $qry = '', $cdvalid = FALSE, $cdreta = FALSE)
		{
			...
			/*
			 ...
			 "SELECT c.*, u.*, ue.* FROM #fanfiction_comments AS c
